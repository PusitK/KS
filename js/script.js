...
var model = {
    ...
    refreshToken: function refreshToken(com, iden) {
        $.ajax({
            crossDomain: true,
    ...
    onloadFetch: function onloadFetch() {
        ...
        $.ajax({
            url: "https://qwmpk1nu5e.execute-api.ap-southeast-1.amazonaws.com/dev/v2/selector/tags?business=" + localStorage.business,
            type: "GET",
            dataType: 'json',
            headers: {
                "Content-Type": 'application/json; charset=utf-8',
...
var view = {
    ...
    countForm: function countForm() {
        ...
        for (var i = 1; i <= 5; i++) {
            ...
            var total = countFilter[0].is_completed.true + countFilter[0].is_completed.false;
            var per_com_true = (countFilter[0].is_completed.true / total * 100).toFixed(2) || 0;
            if (isNaN(per_com_true)) per_com_true = 0;
            var per_com_false = (countFilter[0].is_completed.false / total * 100).toFixed(2) || 0;
    ...
    setColor: function setColor(pos, percent) {
        if (percent > 0 && percent <= 25) {
            pos.style.backgroundColor = "rgb(241,248,255)";
        } else if (percent > 25 && percent <= 50) {
            pos.style.backgroundColor = "rgb(215,236,255)";
        } else if (percent > 50 && percent <= 75) {
            pos.style.backgroundColor = "rgb(190,224,255)";
        } else if (percent > 75 && percent <= 99) {
            pos.style.backgroundColor = "rgb(164,212,255)";
        } else if (percent == 100) {
            pos.style.backgroundColor = "#93bee5";
        }
    },
...
var ctrl = {
    ...
    exportExcel: function exportExcel(com, iden) {
...
function generateColorSummary(vcol, hrow) {
    ...
    for (var z = 0; z < totalelem.length; z++) {
        /// length = 5 all table
        var idTable = document.getElementById('table' + z);
        var eachRow = idTable.querySelectorAll('.sum-row');
        var eachCol = idTable.querySelectorAll('.sum-col');
        ...
        for (var _i3 in eachRow) {
            ...
            if (typeof eachRow[_i3] !== 'undefined' && !isNaN(_percent)) {
...
function createProgMax5(data) {
    ...
    var prog_elem = document.getElementById('progress-question-max5');
    prog_elem.innerHTML = '';
    var sum_prog_elem = document.getElementById('summary-progress-max5');
    sum_prog_elem.innerHTML = '';
    var count = 0;
    var max5_count = 0;

    var sum_score = 0;
    var max5_sum_score = 0;
    ...
    for (var i in data) {
        ...
        if (data[i].que_sum_score == 0) {
            ...
            var percent_value = Math.floor(data[i].que_sum_score / (data[i].doc_count * 5) * 100);
            var max5 = (data[i].que_sum_score / (data[i].doc_count * 5) * 5).toFixed(2);

            max5_sum_score += 5;
            max5_count = max5_count + data[i].que_sum_score / (data[i].doc_count * 5) * 5;
        }

        var progress_element = "<div class=\"form-group\">\n                <label class=\"label-title\"> " + data[i].q_num + ". " + data[i].question[0] + "</label>               <div class=\"progress col-lg-10\" style=\"padding:0\">\n                    <div class=\"progress-bar color-intensity-2\" role=\"progressbar\"                           aria-valuenow=" + percent_value + "aria-valuemin=\"0\" aria-valuemax=\"100\"                         style=\"width:" + percent_value + "%\">                                \n                    <span class=\"progress-value\">" + max5 + "</span>                            \n                    </div> \n                </div>                        \n                    <span class=\"col-lg-2 \">" + max5 + '/5' + "</span>\n                </div>";
        // 5 is max score 
        prog_elem.insertAdjacentHTML('beforeend', progress_element);
    }
    var createSumProgElem = "<div class=\"progress margin-top-1 col-lg-10\" style=\"padding:0\">                   \n                                <div class=\"progress-bar color-intensity-3\" role=\"progressbar\" aria-valuenow=\"" + percent_summary + "\" aria-valuemin=\"0\" aria-valuemax=\"100\" \n                                style=\"width:" + percent_summary + "%\">                        \n                                <span class=\"progress-value\"><b>" + (max5_count / max5_sum_score * 5).toFixed(2) + "</b></span>                  </div>\n                            </div>\n                            <span class=\"col-lg-2 progress-number\">" + (max5_count / max5_sum_score * 5).toFixed(2) + '/5' + "</span>";
    sum_prog_elem.insertAdjacentHTML('beforeend', createSumProgElem);
}

function switchViewProg() {
    var sumProgFull = document.getElementById('summary-progress');
    var sumProgMax5 = document.getElementById('summary-progress-max5');

    var progElemFull = document.getElementById('progress-question');
    var progElemMax5 = document.getElementById('progress-question-max5');
    if (sumProgFull.style.display === 'block') {

        sumProgFull.style.display = 'none';
        progElemFull.style.display = 'none';

        sumProgMax5.style.display = 'block';
        progElemMax5.style.display = 'block';
    } else {
        sumProgFull.style.display = 'block';
        progElemFull.style.display = 'block';

        sumProgMax5.style.display = 'none';
        progElemMax5.style.display = 'none';
    }
}
...
function createall(data, callback) {
    ...
    var containerQuestion = document.getElementById('question-all');
